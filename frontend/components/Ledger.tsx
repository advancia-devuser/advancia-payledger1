    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="text-left">
          <h1 className="text-2xl font-black text-slate-900 uppercase tracking-tight italic">Pay Ledger</h1>
          <p className="text-slate-500 font-medium">Review and manage all corporate financial movements.</p>
        </div>
                ...
                >
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        <p className="text-sm font-black text-slate-900 tracking-tight">{t.description}</p>
                  ...
                  <td className="px-8 py-5">
                    <p className={`text-sm font-black italic tracking-tight ${t.amount > 0 ? 'text-emerald-600' : 'text-slate-900'}`}>
                  ...
                  <td className="px-8 py-5">
                    <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest ${
                      t.status === TransactionStatus.COMPLETED ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' :
                      t.status === TransactionStatus.PENDING ? 'bg-amber-50 text-amber-700 border border-amber-100' :
                      'bg-slate-100 text-slate-600'
                    }`}>
                      <span className={`w-1.5 h-1.5 rounded-full ${
                        t.status === TransactionStatus.COMPLETED ? 'bg-emerald-500' :
                        t.status === TransactionStatus.PENDING ? 'bg-amber-500' :
                        'bg-slate-400'
                      }`}></span>
                      {t.status}
                    </div>
          ...
          <div className="relative w-full max-w-2xl bg-white rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-10 duration-500">
            ...
            <div className="p-10 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              <div className="flex items-center gap-4">
                ...
                <div className="text-left">
                  <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tight italic">Transaction Details</h3>
                  <p className="text-[10px] text-slate-400 font-black uppercase tracking-[0.2em]">Audit ID: {selectedTransaction.id}</p>
                </div>
              </div>
              <button 
                onClick={closeModal}
            ...
            <div className="p-10 space-y-10 max-h-[70vh] overflow-y-auto">
              ...
              <div className="text-center space-y-2 py-6 bg-slate-50 rounded-[2rem] border border-slate-100">
                ...
                <p className={`text-6xl font-black italic tracking-tighter ${selectedTransaction.amount > 0 ? 'text-emerald-500' : 'text-slate-900'}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-6">
                  <div className="flex items-start gap-4">
                    <div className="p-3 bg-indigo-50 text-indigo-600 rounded-xl"><Hash size={18} /></div>
                    <div className="text-left">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Description</p>
                      <p className="text-sm font-black text-slate-900">{selectedTransaction.description}</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-4">
                    <div className="p-3 bg-indigo-50 text-indigo-600 rounded-xl"><Calendar size={18} /></div>
                    <div className="text-left">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Execution Date</p>
                      <p className="text-sm font-black text-slate-900">{selectedTransaction.date}</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-4">
                    <div className="p-3 bg-indigo-50 text-indigo-600 rounded-xl"><Tag size={18} /></div>
                    <div className="text-left">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Category & Type</p>
                      <p className="text-sm font-black text-slate-900">{selectedTransaction.category} • {selectedTransaction.type}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  <div className="flex items-start gap-4">
                    <div className="p-3 bg-indigo-50 text-indigo-600 rounded-xl"><CreditCard size={18} /></div>
                    <div className="text-left">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Network / Processor</p>
                      <p className="text-sm font-black text-slate-900">{selectedTransaction.network || 'Internal Ledger'}</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-4">
                    <div className="p-3 bg-emerald-50 text-emerald-600 rounded-xl"><ShieldCheck size={18} /></div>
                    <div className="text-left">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Integrity Score</p>
                      <p className="text-sm font-black text-emerald-600">VERIFIED (100%)</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white relative overflow-hidden group">
                ...
                <div className="relative z-10 flex items-start gap-5">
                   ...
                   <div className="text-left">
                      <h4 className="text-sm font-black uppercase tracking-tight italic mb-2">Advancia AI Integrity Insight</h4>
                      <p className="text-[11px] text-slate-400 leading-relaxed font-medium">
                        This {selectedTransaction.type} aligns with historical {selectedTransaction.category} velocity. No anomalous behavior detected on {selectedTransaction.network || 'the internal chain'}. SONNET 4.5 suggests marking as "Low Audit Priority".
                      </p>
                   </div>
                </div>
              </div>
