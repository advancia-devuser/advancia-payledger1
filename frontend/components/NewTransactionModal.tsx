...
const NewTransactionModal: React.FC<NewTransactionModalProps> = ({ onClose, onAdd }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newTx: Transaction = {
      ...
      amount: formData.type === TransactionType.INVOICE || formData.type === TransactionType.CRYPTO_SETTLEMENT 
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-6">
      <div className="absolute inset-0 bg-slate-900/80 backdrop-blur-xl animate-in fade-in duration-300" onClick={onClose}></div>
      ...
      <div className="relative w-full max-w-xl bg-white rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-500">
        <div className="p-8 border-b border-slate-100 flex items-center justify-between">
          <div className="text-left">
            <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight italic">New Ledger Entry</h3>
            <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-1">Manual or AI-Assisted Entry</p>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-slate-50 rounded-xl transition-all text-slate-400"><X size={20}/></button>
        </div>
          ...
          <div className="grid grid-cols-1 gap-6">
              ...
              >
                {isScanning ? (
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-xs font-black text-indigo-600 uppercase tracking-widest animate-pulse">Advancia AI Scanning...</p>
                  </div>
                ) : (
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-xs font-black text-slate-500 uppercase tracking-widest">Drop Receipt for AI Auto-Fill</p>
                  </div>
                )}
              </label>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="text-left">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 block">Network</label>
                  <select 
